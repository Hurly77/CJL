...
export default function SocialsAnimatedList(props: { size?: "sm" | "md" | "lg" }) {
  const { size } = props;
  const duration = 0.2;
  const initial = { x: -56, opacity: 0 };

  return SocialIcons.map(({ Icon, z, link }, i) => {
      ...
      >
        <Icon
          className={cls(
            ...
            size === "sm" ? "w-8 h-8 p-1.5" : size == "md" ? "w-10 h-10 p-1.5" : "w-12 h-12 p-2",
          )}
        />
      </motion.a>
    );
